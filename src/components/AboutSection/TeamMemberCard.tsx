...
export function TeamMemberCard({ member, isHighlighted = false, showDescription = false }: TeamMemberCardProps) {
  return (
    <motion.div
      ...
      className={`perspective-1000 transform-gpu ${isHighlighted ? 'col-span-1' : ''}`}
    >
      <Card
        variant={isHighlighted ? 'bordered' : 'glass'}
        className="h-full max-w-sm mx-auto" // 🔥 FIX: compact, centered card
      >
        <div className="relative group overflow-hidden rounded-t-xl">
          <div className="aspect-[4/5] relative">
            <img
              src={member.imageUrl}
              alt={member.name}
              className="absolute inset-0 w-full h-full object-cover object-[50%_35%] transform transition-all duration-500 group-hover:scale-105"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[var(--bg-darker)]/90 via-[var(--bg-dark)]/50 to-transparent opacity-70 group-hover:opacity-90 transition-opacity duration-300" />
